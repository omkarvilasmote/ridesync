...
function OrderDetails() {
    ...
    async function getOrders() {
        try {
            const res = await axios.get("http://localhost:9090/driver-bookingdsdetails", {
                headers: {
        ...
        <div className="py-9">
            <div className="container max-w-[1280px] mx-auto flex justify-center items-center gap-6 h-[650px]">
                <div className="w-[5%] rounded-lg shadow h-[400px] flex flex-col gap-6 justify-center items-center font-bold bg-white">
                    <Link to="/driver-home" className="text-xl">
                        <FaHome />
                    </Link>
                    <Link to="/driver-booking-details" className="text-xl text-red-500">
                        <RxActivityLog />
                    </Link>
                    <Link to="/driver-profile" className="text-xl">
                        <CgProfile />
                    </Link>
                </div>
